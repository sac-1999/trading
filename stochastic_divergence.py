...
class High_volume_surge_on_day(strategy.BaseStrategy):
    ...
    def save_image(self, df, day, peaks, bottoms, entry, sl, maxrr, dayendrr):
        ...
        # ema_plots.append(mpf.make_addplot(df['stochk_60_3'], panel = 2,color='red', width=4)) 
    ...
    def trade_strategy(self, df, newpeaks, newbottoms, prevdaydf):
        ...
        df['ema_small'] = (df['ema_100']>df['ema_100'].shift(1)) & (df['ema_100'].shift(1)>df['ema_100'].shift(2)) & (df['ema_100'].shift(2)>df['ema_100'].shift(3))
        df['ema_big'] = (df['ema_200']>df['ema_200'].shift(1)) & (df['ema_200'].shift(1)>df['ema_200'].shift(2)) & (df['ema_200'].shift(2)>df['ema_200'].shift(3))
        df['breakout1'] = (df['stochk_9_3'] < 20) & (df['stochk_14_3'] < 20) &(df['stochk_40_3'] < 20) &(df['stochk_60_3'] < 20)
        df['breakout2'] = df['breakout1'] & df['breakout1'].shift(1) & df['breakout1'].shift(2) & (df['time'] < pd.to_datetime('13:00').time())
        df['breakout2'] = df['breakout2'].cummax() 
        df['breakout'] =  df['breakout2'] & (df['close'] > df['high'].shift(1)) & (df['time'] < pd.to_datetime('11:30').time())
        df = df[df['breakout']]
        if len(df) == 0:
            return 
        traderow = df.iloc[0]

        if not traderow['ema_small'] or  not traderow['ema_big']:
            return 
    ...
    def apply_strategy(self):   
        ...
        for x_day in alldays:
            if (datetime.today() - timedelta(100)).date() >= pd.to_datetime(x_day).date():
                prevday = x_day
                continue
            newprevday = pd.to_datetime(prevday) - timedelta(1)
