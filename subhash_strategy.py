...
end_date = datetime(2025, 9, 30, 15, 30)
interval = "5min"
all_results = []
save_image_tmp =  True
s_r_timeframe = '15min'
...
class High_volume_surge_on_day(strategy.BaseStrategy):         
    ...
    def trade_strategy(self, df, newpeaks, newbottoms, prevdaydf):
        ...
        if self.trade_type=='buy':
            ...
            # df = self.Intraday_breakouts(df, 5)
            df['activate'] = (df['change'] >= 1) & (df['time'] < pd.to_datetime('11:00').time())
            df['activate'] = df['activate'].cummax()
            df['max'] = df['high'].cummax()
            df['maxclose'] = df['close'].cummax()
            df['breakout'] = df['activate'] & (df['time'] > pd.to_datetime('09:20').time()) & (df['close'] > df['high'].shift(1)) & (df['close'] > df['vwap']) & (df['low'] < df['vwap']) & (df['low'] <= df['low'].shift(1))
            df['entry'] = df['close']
            ...
            sl = sl - 0.001 * sl
            # sl = entry - entry * 0.00
            if  (traderow['time'] > pd.to_datetime('11:00').time())  or (traderow['maxclose'] < prevdaydf['high'].max()):
                return 
