...
class High_volume_surge_on_day(strategy.BaseStrategy):
    ...
    def fake_breakout_trap(self, df):
        df = self.Intraday_breakouts(df,5)
        df['max'] = df['high'].cummax()
        rule1 = (df['high']> df['high'].shift(1)) & (df['close']<df['low'].shift(1)) & ((df['max'] == df['high']) | (df['max'] == df['high'].shift(1))) 
        # rule2 = (df['high'].shift(1) == df['max']) & (df['close'] < df['low'].shift(1))
        df['trapbreakout'] = (rule1 ) & (df['time'] < pd.to_datetime('14:00').time()) & (df['time'] > pd.to_datetime('09:35').time())
        return df
    ...
    def trade_strategy(self, df, newpeaks, newbottoms, prevdaydf):
        ...
        df = self.fake_breakout_trap(df)
        df['reversalpoint'] = df['trapbreakout']
        df['sl'] = df['max']
        breakdf = df[df['reversalpoint']]
        if len(breakdf) == 0:
            return 
