...
exchange = 'NSE'
start_date = datetime(2025, 5, 2, 9, 10)
end_date = datetime(2025, 9, 27, 9, 10)
interval = "10min"
all_results = []
save_image_tmp =    True
s_r_timeframe = '30min'
level_window = 3
maxnumber_of_levels = 3
save_by_stock = False
draw_levels = False
islive = True
...
class High_volume_surge_on_day(strategy.BaseStrategy):         
    ...
    def strategy2_morning_buy_vwap(self, df):
        ...
        df['goodclose'] = df['close'] > (df['high'] - (df['high'] - df['low']) * 0.1)
        df['breakout1'] = (df['vwapcandle'].shift(1)) & (df['low'] < df['high'].shift(1)) & (df['close'])
        df['entry1'] = df['close'].shift(1)
        df['sl1'] = df['low'].shift(1)
        df['goodvolume1'] = df['volume'].shift(1) > df['volume'].shift(2)
    ...
    def trade_strategy(self, df, newpeaks, newbottoms, prevdaydf):
        ...
        if self.trade_type=='buy':
            ...
            sl = traderow['sl']
            # sl = min(sl, entry - entry * 0.004)
            # goodvolume = traderow['goodvolume']
            # slpct = (entry - sl)/sl*100

            # if slpct < 0.2 or slpct > 0.8:
            #     return 
            # if not goodvolume  or entry < prevdaydf['high'].max() or (traderow['time'] < pd.to_datetime('09:40').time()) or (traderow['time'] > pd.to_datetime('11:30').time()) or not traderow['goodclose']:
            #     return 

            if  (traderow['time'] > pd.to_datetime('12:00').time()) or not traderow['goodclose']:
                return 
        ...
        if self.trade_type == 'sell':
            if len(df) == 0:
                return 
            n = 3
            df = self.Intraday_breakdowns(df, n)
            df['goodvolume'] = (df['volume'] > df['volume'].shift(1))
            df['goodclose'] = (df['close'] < (df['low'] + 0.2 * (df['high'] - df['low'])))
            df['breakdown'] = (df['close'] < df['support']) & (df['support'] == df['min'].shift(n))
            df['resistance'] = np.where((df['high'] > df['high'].shift(-1)) & (df['high'] > df['high'].shift(1)) & (df['high'] > df['high'].shift(2)), df['high'], np.nan)
