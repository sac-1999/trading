...
class High_volume_surge_on_day(strategy.BaseStrategy):
    ...
    def trade_strategy(self, df, newpeaks, newbottoms, prevdaydf):
        ...
        df['breakout'] = (df['low'] < df['low'].shift(1)) & (df['close'] > df['high'].shift(1)) & (df['volume'] > df['volume'].shift(1)) & (df['time'] < pd.to_datetime('12:00').time()) & (df['time'] >= pd.to_datetime('09:30').time())
        df['sl'] = df['low']
        df['entry'] = df['close']
