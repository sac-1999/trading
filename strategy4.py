...
class High_volume_surge_on_day(strategy.BaseStrategy):         
    ...
    def find_relevant_levels(self, x_day):
    ...
    def pinbar(self, df):
        df['min'] = df['low'].cummin()
        df['body'] = abs(df['high'] - np.minimum(df['close'], df['open']))
        df['wick'] = abs(np.minimum(df['close'], df['open']) - df['low'])
        df['pinbar'] = df['wick'] > df['body'] * 2
        df['fullbody'] = df['high'] - df['low']
        df['volumefavour'] = (df['volume'] > df['volume'].shift(1))
        df['volumefavour'] = df['volumefavour'].shift(1) 
        df['breakout1'] = df['pinbar'] & (df['fullbody'] > 1 * df['atr'])
        df['breakout'] = df['breakout1'].shift(1) & (df['high'] > df['high'].shift(1))
        return df
    
    def invisible_pinbar(self,df):
        df['open_m'] = df['open'].shift(1)
        df['close_m'] = df['close']
        df['low_m'] = np.minimum(df['low'], df['low'].shift(1))
        df['high_m'] = np.maximum(df['high'], df['high'].shift(1))
        df['min_m'] = df['low_m'].cummin()
        df['body'] = abs(df['high_m'] - np.minimum(df['close_m'], df['open_m']))
        df['wick'] = abs(np.minimum(df['close_m'], df['open_m']) - df['low_m'])
        df['pinbar_m'] = (df['wick'] > df['body'] * 2)
        df['fullbody'] = df['high_m'] - df['low_m']
        df['volumefavour'] = (df['volume'] > df['volume'].shift(1))
        df['volumefavour'] = df['volumefavour'].shift(1) 
        df['breakout1'] = df['pinbar_m'] & (df['fullbody'] > 1 * df['atr'])
        df['breakout'] = df['breakout1'].shift(1) & (df['high'] > df['high'].shift(1))
        return df
    
    def down_pinbar(self, df):
        df['max'] = df['high'].cummax()
        df['body'] = abs(df['low'] - np.maximum(df['close'], df['open']))
        df['wick'] = abs(np.maximum(df['close'], df['open']) - df['high'])
        df['pinbar'] = df['wick'] > df['body'] * 2
        return df
    ...
    def trade_strategy(self, df, newpeaks, newbottoms, prevdaydf):
        ...
        if self.trade_type == 'buy':
            ...
            if not traderow['volumefavour'] or traderow['entry'] < prevdaydf['high'].max():
                return 
            
            if traderow['time'] < pd.to_datetime('10:00').time() or traderow['time'] > pd.to_datetime('13:00').time():
                return      
        ...
        if self.trade_type == 'sell':
            df = self.down_pinbar(df)
            df['fullbody'] = df['high'] - df['low']
            df['volumefavour'] = (df['volume'] > df['volume'].shift(1)) 
            df['volumefavour'] = df['volumefavour'].shift(1)
            df['breakout1'] = df['pinbar'] & (df['fullbody'] > 1 * df['atr'])
            df['breakout'] = df['breakout1'].shift(1) & (df['low'] < df['low'].shift(1))
            df['entry'] = df['low'].shift(1)
            df['sl'] = df['high'].shift(1)
            breakdf = df[df['breakout']]
    ...
    def apply_strategy(self):   
        ...
        for i, x_day in enumerate(alldays):
            ...
            if islive:
                ...
                print(dftmp)
            if draw_levels:
                levels = self.find_relevant_levels(x_day)
