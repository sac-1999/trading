...
class High_volume_surge_on_day(strategy.BaseStrategy):
    ...
    def trade_strategy(self, df, newpeaks, newbottoms, prevdaydf):
        ...
        ema = 'ema_8'
        df['ema_failed'] = df['close'] < df[ema]
        df.loc[0,'ema_failed'] = False
        # df['ema_failed'] = df['ema_failed'].cummax()
        # df['breakout'] = (df['ema_failed'].shift(1) == False) & (df['close'] > df['high'].shift(1)) & ((df['low'] < df[ema]) & (df['low'].shift(1) < df[ema].shift(1))) & (df['time'] <= pd.to_datetime('11:00').time()) & (df['high'] > prevdayhigh)
       
        df['islowest'] = df['low'] == df['low'].cummin()
        df['breakout'] = (df['close'] > df['high'].shift(1)) & (df['islowest'] | df['islowest'].shift(1)) & (df['open'] < df['close'].shift(1) - df['close']*0.00004) & (df['time'] >= pd.to_datetime('09:40').time()) & (df['time'] <= pd.to_datetime('13:00').time())  & (df['close'].shift(1) < df['open'].shift(1)) & (df['close'] < prevdaylow)
        df['sl'] = np.minimum(df['low'], df['low'])
