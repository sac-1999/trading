...
exchange = 'NSE'
start_date = datetime(2025, 1, 20, 9, 10)
...
for symbol in pd.read_csv('ind_nifty50list.csv')['Symbol']:
    try:
        ...
        daydf = daydf[['day', 'previous_day_close']]
        df = pd.merge(df, daydf, on = 'day', how = 'left')
        df['change_pct'] = round((df['close'] - df['previous_day_close']) / df['previous_day_close'] * 100 , 2)
...
while(True):
    for symbol in pd.read_csv('ind_nifty50list.csv')['Symbol']:
        try:
            ...
            mystrategy = strategy.VWAP_by_YatinKoodi(orgdf.copy(), 'buy', window = 3, ema_list = [8, 21, 50, 100], symbol = symbol, save_trade = save_image, live = is_live, change_dict = changedf_dict)
            results = mystrategy.run()
            all_results.extend(results)
            mystrategy = strategy.VWAP_by_YatinKoodi(orgdf.copy(), 'sell', window = 3, ema_list = [8, 21, 50, 100], symbol = symbol, save_trade = save_image, live = is_live, change_dict = changedf_dict)
            results = mystrategy.run()
            all_results.extend(results)
            # break
        except Exception as e:
